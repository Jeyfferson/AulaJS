...
const horaExercicio = 11
if(horaExercicio < 12)
{
  console.log('Bom dia!')
}
else if(horaExercicio < 18)
{
...
if(myOld <= 7 || myOld >= 65)
{
  priceMessenger = 'Para você a entrada é grátis'
}
else if(myOld > 7 && myOld <= 17)
{
  priceMessenger = 'Você terá desconto de 50% no ingresso'
}
else
{
  priceMessenger = 'A entrada é de R$ 30,00'
}

console.log(priceMessenger)
...
{
  number = numbers[i] // Apenas para melhorar a leitura
...
let numbersCrazy = numbersBoole = numbersString = 0
...
console.log(`O crazyArray tem ${numbersBoole} booleans, ${numbersCrazy} números e ${numbersString} strings.`)
...
{
  const positionI = randomNumbers[i]
  const isEveNumber = positionI%2 == 0 

  if(positionI == 0)
  {
    console.log('Um número 0')
  }
  else if (isEveNumber) //como se fosse "isEveNumber == True"
  {
    arrayPar.push(positionI)
  }else
  {
    arrayImp.push(positionI)
  }
}

const arrayParString = arrayPar.join(', ').replace(', 5',' e 5')
const arrayImpString = arrayImp.join(', ').replace(', 3',' e 3')

console.log(`Numeros ímpares: ${arrayImpString}. Números pares: ${arrayParString}`)
